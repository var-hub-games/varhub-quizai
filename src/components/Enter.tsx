...
export const Enter: FC<{onCreate: (data: VarhubGameClient) => void}> = (props) => {
	...
	const [room, setRoom] = useState(initValues.room as string|undefined);
	const [url, setUrl] = useState(initValues.url as string|undefined);
	const [name, setName] = useState(initValues.name as string|undefined);

	const enterRoom = useCallback(async (url: string, room:string, clientName:string, chatGptUrl: string|undefined) => {
		const ctrl = new AbortController();
		try {
			setAbortCtrl(ctrl);
			const client = await createRoomAndClient(url, room, clientName, ctrl, {chatGptUrl});
	...
	useEffect(() => {
		...
		if (initValues.url && initValues.room && initValues.name && initValues.join) {
			void enterRoom(initValues.url, initValues.room, initValues.name, undefined);
		}
	}, []);
	...
	const selectRoom = useCallback((room: string) => {
		setRoom(room);
		if (url && name) void enterRoom(url, room, name, undefined);
	}, [url, name])
	...
	return (
		<form onSubmit={onSubmit}>
			...
			<div>
				<input disabled={!!abortCtrl} name="chatGptUrl" type="text" placeholder="Chat GPT url"/>
			</div>
...
async function createRoomAndClient(url: string, roomId: string, name: string, ctrl: AbortController, config: any){
	const hub = new Varhub(url);
	if (!roomId) {
		const { roomModule, roomIntegrity} = await import("varhub-modules:../controllers:index.ts");
		const roomData = await hub.createRoom(roomModule, {integrity: roomIntegrity, config});
		roomId = roomData.id;
